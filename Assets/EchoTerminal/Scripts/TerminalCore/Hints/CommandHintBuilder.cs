...
namespace EchoTerminal
{
    public static class CommandHintBuilder
    {
            ...
            Tokenizer tokenizer)
        {
            ...
            segments.Add(new CommandHintSegment(commandName, segmentIndex++ == activeSegmentIndex));

            if (entry.HasTarget)
            {
                segments.Add(new CommandHintSegment("@target", segmentIndex++ == activeSegmentIndex));
            }
            ...
            foreach (ParameterInfo param in entry.Method.GetParameters())
            {
                ...
                segments.Add(new CommandHintSegment(label, segmentIndex++ == activeSegmentIndex));
            }
        ...
        private static List<CommandEntry> ResolveCandidates(CommandParseResult result)
        {
            ...
            {
                return new List<CommandEntry> { result.Entry.Value };
            }
        ...
        {
            if (tokenizer != null && tokenizer.TryGetParser(param.ParameterType, out ITokenParser parser) &&
